...
class DDPG(object):
    ...
    def _init_state_importance(self):
        if hasattr(self.env, 'spec'):
            if hasattr(self.env.spec, 'id'):
                if "ContinuousSpaceMaze" in self.env.spec.id:
                    # ContinuousSpaceMaze
                    self.resolution = 25
                    self.test_states = np.array(
                        [[i, j] for j in range(0, self.resolution * 2, 2) for i in range(0, self.resolution * 2, 2)])
        else:
            # MountainCarContinuous
            self.resolution = 25
            x = np.linspace(self.env.env.low_state[0], self.env.env.high_state[0], self.resolution)  # position
            y = np.linspace(self.env.env.low_state[1], self.env.env.high_state[1], self.resolution)  # velocity
            self.test_states = np.array([[[x[j], y[i]] for j in range(self.resolution)] for i in range(self.resolution)]).reshape(-1, 2)

        self.test_states = self.test_states.astype(dtype=np.float32)
    ...
    def calc_knack(self, observations, calc_map=False):
        ...
        if observations.shape[0] == 1:
            # calc knack for a state
            _observation = np.array([observations[0] for i in range(self.test_N)])
        ...
        return knack_map, knack_map_kurtosis, q_1_moment