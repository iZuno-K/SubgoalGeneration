...
def build_train(make_obs_ph, q_func, num_actions, optimizer, grad_norm_clipping=None, gamma=1.0,
                ...
                exploitation_ratio_on_bottleneck=None, bottleneck_threshold_ratio=None):
...
`       See the top of the file for details.
    ...
    elif exploitation_ratio_on_bottleneck is not None and bottleneck_threshold_ratio is not None:
        print("Use bottleneck exploration")
        act_f = build_act_based_bottleneck(make_obs_ph, q_func, num_actions, exploitation_ratio_on_bottleneck,
                                           bottleneck_threshold_ratio, scope=scope, reuse=reuse)
    else:
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if exploitation_ratio_on_bottleneck is not None and bottleneck_threshold_ratio is not None:
            bottleneck_values_t = build_calc_bottleneck(q_t)
            bottleneck_values = U.function([obs_t_input], bottleneck_values_t)

            return act_f, train, update_target, {'q_values': q_values, 'bottleneck_values': bottleneck_values}
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        update_eps_ph = tf.placeholder(tf.float32, (), name="update_eps")
        # done! 閾値が必要
        bottleneck_threshold_ph = tf.placeholder(tf.float32, (), name="bottleneck_threshold")

        eps = tf.get_variable("eps", (), initializer=tf.constant_initializer(0))

        q_values = q_func(observations_ph.get(), num_actions, scope="q_func")
        deterministic_actions = tf.argmax(q_values, axis=1)
        # done! bottleneck度を計算する必要
        bottleneck_value = build_calc_bottleneck(q_values)

        # done! イプシロンの計算
        target_eps = KnackBasedQlearnig.calc_epsilon(bottleneck_exploitation_ratio, exploitation_ratio, update_eps_ph)
        update_eps_expr = eps.assign(tf.cond(update_eps_ph >= 0, lambda: target_eps, lambda: eps))
